...
/* An option carrying an illegal length byte (< 2) must not end the option scan
 * early and let a source route behind it reach a local socket. */
START_TEST(test_ip_recv_drops_source_routed_packet_undersized_option_hides_ssrr)
{
    struct wolfIP s;
    int listen_sd;
    struct tsocket *listener;
    struct wolfIP_sockaddr_in sin;
    uint8_t pkt[ETH_HEADER_LEN + 32 + TCP_HEADER_LEN];
    struct wolfIP_ip_packet *ip;
    struct wolfIP_ll_dev *ll;
    union transport_pseudo_header ph;
    uint16_t *tcp_csum_field;
    ...
    /* IHL=8 for 12 bytes of IP options. */
    ip->ver_ihl = 0x48;
    ip->ttl = 64;
    ip->proto = WI_IPPROTO_TCP;
    ip->len = ee16(32 + TCP_HEADER_LEN);
    ip->src = ee32(0x0A0000A1U);
    ip->dst = ee32(0x0A000001U);

    {
        ...
        opts[2]  = 0x89;  /* SSRR hidden behind the undersized stub */
        opts[3]  = 7;     /* well formed: type + length + pointer + one route */
        opts[4]  = 4;     /* pointer (RFC 791) */
        opts[5]  = 10; opts[6] = 0; opts[7] = 0; opts[8] = 9;
    ...
    {
        uint8_t *tcp = pkt + ETH_HEADER_LEN + 32;
        tcp[0] = (uint8_t)(40000 >> 8);
        tcp[1] = (uint8_t)(40000 & 0xFF);
        tcp[2] = (uint8_t)(1234 >> 8);
        tcp[3] = (uint8_t)(1234 & 0xFF);
    ...
    /* Listener must stay in LISTEN: with the buggy parser the walk breaks out
     * on the length-1 stub, the SSRR behind it is never examined and the SYN
     * flips the listener to TCP_SYN_RCVD. */
    ck_assert_int_eq(listener->sock.tcp.state, TCP_LISTEN);
}
END_TEST
